...
class UlanziStreamDeck {
  ...
  disconnect() {
    ...
    try {
      ...
      Utils.warn("websocket disconnect failed", error);
    }
  }
  ...
  logMessage(msg, level) {
    if (!Utils.isDebugEnabled()) return this;
  ...
  setSettings(settings, context) {
    Utils.log("setSettings", { settings, context });
    const { uuid, key, actionid } = context ? this.decodeContext(context) : {};
...
window.addEventListener("beforeunload", () => $UD.disconnect(), { once: true });
window.addEventListener("pagehide", () => $UD.disconnect(), { once: true });
