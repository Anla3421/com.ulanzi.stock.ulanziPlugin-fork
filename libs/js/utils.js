...
class UlanziUtils {
		...
		Array.from(elements)
			...
			.forEach((element) => {
				...
				if (isCheckOrRadio) {
					...
					this.log('setFormValue isSingle', { isSingle, value, elementValue: element.value });
   ...
   */
	...
	*/
	async readJson(path) {
		if (!path) {
			this.error('A path is required to readJson.');
		}
		...
		return new Promise((resolve, reject) => {
			try {
				...
				req.onerror = finishReject;
				req.ontimeout = () => finishReject(new Error('Read JSON timed out'));
				req.timeout = 5000;
				...
				req.onreadystatechange = (response) => {
					if (req.readyState === 4) {
						...
						if (jsonString) {
							try {
								finishResolve(JSON.parse(jsonString));
							} catch (e) {
								finishReject(e);
							}
						} else {
							finishReject(new Error('Empty JSON response'));
						}
					}
				};
   ...
   */
	loadImagePromise(url) {
		return new Promise(resolve => {
			...
			timeoutId = window.setTimeout(() => {
				img.src = '';
				finish({ url, status: 'error' });
			}, 15000);
			img.onload = () => finish({ url, status: 'ok', img });
			img.onerror = () => finish({ url, status: 'error' });
			img.src = url;
		});
	}
	...
	getData(url, param) {
		...
		return new Promise(function (resolve, reject) {
			...
			req.ontimeout = function () {
				Utils.error('Request timed out');
				finishReject(Error('Request timed out'));
			};
			...
			req.onload = function () {
				...
				if (req.status === 200) {
					...
					finishResolve(req.response);
				} else {
					// console.warn('=====getData not 200:')
					finishReject(Error(req.statusText));
				}
			};

			req.onerror = function () {
				// console.warn('=====getData error:')
				finishReject(Error('Network Error'));
			};
   ...
   */
	...
	isDebugEnabled() {
		const queryDebug = String(this.getQueryParams('debugAll') || '').trim().toLowerCase();
		let storageDebug = '';
		try {
			storageDebug = String(window.localStorage && window.localStorage.getItem('ULANZI_PLUGIN_DEBUG_ALL') || '').trim().toLowerCase();
		} catch (error) {
			storageDebug = '';
		}
		if (DEBUG_FALSE_VALUES.has(queryDebug) || DEBUG_FALSE_VALUES.has(storageDebug)) return false;
		if (DEBUG_TRUE_VALUES.has(queryDebug) || DEBUG_TRUE_VALUES.has(storageDebug)) return true;
		return false;
	}
